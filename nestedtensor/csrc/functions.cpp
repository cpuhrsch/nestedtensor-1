...
Tensor& NestedTensor_dropout_(Tensor& input, double p, bool train) {
  input.copy_(NestedTensor_dropout(input, p, train));
  return input;
}
...
Tensor NestedTensor_max_pool2d(
  ...
  if (is_tensor_shape(self)) {
    if (self.is_contiguous()) {
      auto tensor = torch::reshape(nt.get_buffer(), self_impl->sizes());
    ...
    auto res = at::max_pool2d(
        at::stack(tensors), kernel_size, stride, padding, dilation,
        ceil_mode);
...
Tensor NestedTensor_sum(const Tensor& self, c10::optional<ScalarType> dtype)
{
  ...
  if (self_data.is_contiguous()) {
    return at::sum(self_data.get_buffer(), dtype);
  }
...
Tensor NestedTensor_all(const Tensor& self) {
  ...
  return self_impl.get_buffer().all();
}
...
Tensor NestedTensor_any(const Tensor& self) {
  ...
  return self_impl.get_buffer().any();
}
...
Tensor NestedTensor_flatten(
  ...
  int64_t nested_dim = self_data.nested_dim();
  TORCH_CHECK(
      start_dim >= nested_dim, "Cannot flatten nested dimension ",
      start_dim);
  TORCH_CHECK(
