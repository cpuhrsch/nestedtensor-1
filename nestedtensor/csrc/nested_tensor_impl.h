...
struct NestedTensorImpl : public c10::TensorImpl {
  ...
  const caffe2::TypeMeta get_nt_dtype() const {
    return _data_type;
  }
  const caffe2::TypeMeta get_nt_numel() const {
    return _data_type;
  }
  const caffe2::TypeMeta get_nt_is_contiguous() const {
  ...
  const bool get_nt_is_cuda() const {
...
inline int64_t get_dim(const at::Tensor& tensor) {
  if (is_nested_tensor_impl(tensor)) {
    return get_efficient_nested_size(tensor).dim();
  }
  return tensor.dim();
}
...
inline const caffe2::TypeMeta get_dtype(const at::Tensor& tensor) {
  if (is_nested_tensor_impl(tensor)) {
    return get_nested_tensor_impl(tensor)->get_nt_dtype();
  }
  return tensor.dtype();
}
...
inline int64_t get_numel(const at::Tensor& tensor) {
  if (is_nested_tensor_impl(tensor)) {
    return get_efficient_nested_size(tensor).numel();
  }
  return tensor.numel();
}
...
inline bool get_is_contiguous(
  ...
  if (is_nested_tensor_impl(tensor)) {
    return get_nested_tensor_impl(tensor)->get_nt_is_contiguous();
  }
...
inline bool get_is_cuda(
    ...
    at::MemoryFormat memory_format = MemoryFormat::Contiguous) {
  if (is_nested_tensor_impl(tensor)) {
    return get_nested_tensor_impl(tensor)->get_nt_is_cuda();
  }
